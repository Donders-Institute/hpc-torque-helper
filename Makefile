...
build: build_dep
	GOPATH=$(GOPATH) GOOS=linux go install github.com/Donders-Institute/hpc-torque-helper/...

doc:
	@GOPATH=$(GOPATH) GOOS=linux godoc -http=:6060

test: build_dep
	@GOPATH=$(GOPATH) GOOS=linux GOCACHE=off go test -v github.com/Donders-Institute/hpc-torque-helper/test/...
